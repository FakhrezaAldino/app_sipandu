...
interface EditPrestasiModalProps {
    isOpen: boolean;
    onClose: () => void;
    prestasi: any;
    onSuccess?: () => void;
...
const EditPrestasiModal: React.FC<EditPrestasiModalProps> = ({ isOpen, onClose, prestasi, onSuccess }) => {
    ...
    useEffect(() => {
        if (prestasi && isOpen) {
            setNamaAnak(prestasi.namaAnak || '');
            setNamaPrestasi(prestasi.namaPrestasi || '');
            setJenisPrestasi(prestasi.jenisPrestasi || 'Akademik');
            setTingkat(prestasi.tingkat || 'Kecamatan');
            setDeskripsi(prestasi.deskripsi || '');
        }
    }, [prestasi, isOpen]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!namaPrestasi) {
            setError('Nama prestasi wajib diisi');
            return;
        }
        ...
        setError(null);
        try {
            await inputDataService.prestasi.update(prestasi.id, {
                namaAnak,
                namaPrestasi,
                jenisPrestasi,
                tingkat,
            ...
            setError(err.message || 'Gagal mengupdate data prestasi');
        } finally {
            ...
            <div className="bg-white dark:bg-[#1a202c] w-full max-w-[720px] max-h-[90vh] flex flex-col rounded-xl shadow-2xl overflow-hidden ring-1 ring-[#e5e7eb] dark:ring-[#2a3441]">
                ...
                <div className="flex flex-col border-b border-[#f3f4f6] dark:border-[#2a3441] px-6 py-5 bg-white dark:bg-[#1a202c] sticky top-0 z-10">
                    <div className="flex justify-between items-start mb-3">
                        <h2 className="text-[#111418] dark:text-white text-2xl font-bold">Edit Data Prestasi</h2>
                    ...
                    <div className="flex gap-3 flex-wrap">
                        <div className="flex h-8 items-center gap-x-2 rounded-lg bg-[#f0f2f4] dark:bg-[#2a3441] px-3">
                            <span className="material-symbols-outlined text-[#617589] dark:text-gray-400 text-[18px]">person</span>
                            <p className="text-[#111418] dark:text-gray-200 text-sm font-medium">{prestasi?.kpm?.namaLengkap || prestasi?.kpm?.nama || 'KPM'}</p>
                        </div>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 sm:p-8 flex flex-col gap-6 bg-white dark:bg-[#1a202c]">
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t border-[#f3f4f6] dark:border-[#2a3441]">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-8 py-2.5 rounded-lg bg-primary hover:bg-blue-600 text-sm font-bold text-white shadow-md disabled:opacity-50 transition-all"
                        >
...
export default EditPrestasiModal;
